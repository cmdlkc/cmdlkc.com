/* ==========================================================================
   STUDIO KARBON — AXONOMETRIC LOGO UNDER GLASS  v3.0  — TUNABLES
   --------------------------------------------------------------------------
   Loaded by logo-torus-v3.0.html. Both files must sit in the same folder.
   ...
   v3.0 = v2.3 with the liquid-glass cursor over the whole thing.

   THE ONE THING THAT EXPLAINS THE REST: glass is colourless, and the only
   reason you can see it is that it BENDS what is behind it. The glass pass
   reads the page as a texture and samples it with the coordinates pushed
   sideways wherever the liquid is sloped. So anything you want bent has to be
   IN that texture — and live HTML cannot be. Everything on screen is
   therefore drawn into a canvas: the logo, the menu, the previews, the
   project feed, its images, its captions. There is no HTML on this page.

   Two consequences worth knowing before you tune anything:
   · The feed scrolls by a number, not by a scrollbar. `motion.scrollEase`.
   · Section 13 (GLASS) is the whole of the old liquid-glass config, moved in
     wholesale. Its numbers mean exactly what they meant there.
   · Set `glass.enabled: false` and you get v2.3's flat page back, same
     drawing code, no refraction. Useful for telling the two apart when
...
const CONFIG = {
  ...
  shade: {
    ...
    // 0.55 = current — clearly filled, still obviously a dither
  ...
  /* ------------------------------------------------------------------------
     13. THE GLASS
     The whole of the liquid-glass prototype's config, moved in unchanged.
     Every number here means exactly what it meant in that file.
     --------------------------------------------------------------------- */
  glass: {

    // false = no refraction at all. The same page, drawn the same way, blitted
    // straight to the screen — which is v2.3's look. Worth flipping when
    // something looks wrong, to find out which half it is in.
    enabled: true,
    ...
    // ---- Speed gate: nothing happens unless you're actually moving ----------

    // Below this speed the pointer leaves the liquid completely alone.
    // Measured in screen-widths per second, so 0.2 means "crossing the full
    // width of the window would take five seconds".
    //   Range 0–1.5.  0 = always active; holding still pumps liquid into one
    //                     spot until it blows out into a white blob.
    //                 0.18 = current. A slow, deliberate drift leaves no mark.
    //                 0.6 = you have to move briskly before anything happens.
    stillSpeed: 0.18,

    // The speed at which the effect reaches full strength. Between this and
    // `stillSpeed` everything ramps up smoothly — the size of the disturbance
    // is a consequence of how hard you moved.
    //   Range 0.4–6.  1.0 = reaches full effect almost as soon as it starts.
    //                 1.3 = current. An ordinary brisk sweep reaches full effect.
    //                 4.5 = only a hard flick gets the full effect.
    fullSpeed: 1.3,

    // How much the blob is stretched along the direction of travel. 1 is a
    // circle; higher values lay down a streak, so what's left behind reads as
    // the path the pointer cut rather than a disc following it around.
    //   Range 1–8.    1 = round; reads as a blob attached to the cursor.
    //                 3.2 = current. A clear wake along the path.
    //                 6 = a long thin scratch.
    streak: 3.2,

    // How much liquid the pointer pushes out from under itself, at full speed.
    // This is the bulge that gets carried away and stretched by the flow.
